...
- (void)viewDidLoad {
    ...
    _idTextField = [[UITextField alloc] initWithFrame:CGRectMake(10, JKWDeviceHeight * 0.025, JKWDeviceWidth * 0.6, JKWDeviceHeight * 0.05)];
    _idTextField.backgroundColor = [UIColor whiteColor];
    _idTextField.placeholder = @"请输入要删除的学生ID(8位数字)";
    _idTextField.delegate = self;
    _idTextField.keyboardType = UIKeyboardTypeNumberPad;
    [bottomView addSubview:_idTextField];
    
    UIButton *confirmButton = [[UIButton alloc] initWithFrame:CGRectMake(JKWDeviceWidth * 0.65, JKWDeviceHeight * 0.025, JKWDeviceWidth * 0.15, JKWDeviceHeight * 0.05)];
    ...
    [confirmButton setTitle:@"查看" forState:UIControlStateNormal];
    [confirmButton addTarget:self action:@selector(confirmPass) forControlEvents:UIControlEventTouchUpInside];
    [bottomView addSubview:confirmButton];
    
    UIButton *deleteButton = [[UIButton alloc] initWithFrame:CGRectMake(JKWDeviceWidth * 0.85, JKWDeviceHeight * 0.025, JKWDeviceWidth * 0.15, JKWDeviceHeight * 0.05)];
    deleteButton.backgroundColor = [UIColor colorWithRed:0.49f green:0.79f blue:1.00f alpha:1.00f];
    deleteButton.titleLabel.numberOfLines = 0;
    [deleteButton setTitle:@"删除" forState:UIControlStateNormal];
    [deleteButton addTarget:self action:@selector(deletePass) forControlEvents:UIControlEventTouchUpInside];
...
- (void) deletePass {
    ...
    if (j == -1) {
        ...
        [_studentSystem deleteStudentArray:j];
        [_tableView reloadData];
        
        UIAlertController *successAlertController = [UIAlertController alertControllerWithTitle:@"成功" message:@"该生信息已被删除" preferredStyle:UIAlertControllerStyleAlert];
        UIAlertAction *returnAction = [UIAlertAction actionWithTitle:@"返回" style:UIAlertActionStyleCancel handler:nil];
        
        [successAlertController addAction:returnAction];
...
- (void)leftBarBtnClicked:(UIButton *)btn
{
    ...
    if ([_deleteViewControllerDelegate respondsToSelector:@selector(passDelegateNSM:)]) {
        [_deleteViewControllerDelegate passDelegateNSM:_studentSystem.studentNSM];
    }
}
